...
class FossId internal constructor(
        ...
        val result = runBlocking {
            try {
                ...
                if (config.waitForResult && provenance is RepositoryProvenance) {
                    ...
                    val rawResults = getRawResults(scanCode, packageSnippetChoice)
                    createResultSummary(
                        startTime,
                        provenance,
                        rawResults,
     ...
     */
    private suspend fun getRawResults(scanCode: String, packageSnippetChoice: PackageSnippetChoice?): RawResults {
        ...
        pendingFiles += listUnmatchedSnippetChoices(
            ...
            newPendingFiles.map {
                logger.info {
                    "Marked as identified file '$it' is not in .ort.yml anymore: putting it again as 'pending'."
                }
        ...
        val snippets = runBlocking(Dispatchers.IO) {
            pendingFiles.map {
                async {
                    ...
                    val filteredSnippets = snippets.filterNotTo(mutableSetOf()) { it.matchType in excludedMatchTypes }
