...
class NuGetFunTest : StringSpec() {
    init {
        ...
        "Project dependencies are detected correctly" {
            ...
            testNugetInspector(result, expectedResultFile, definitionFile)
        }
    ...
    private fun testNugetInspector(
        result: ProjectAnalyzerResult,
        expectedResultFile: File,
        definitionFile: File
    ) {
        // This will recreate the output file for tests
        if (REGEN) {
            val vcsProcessed = result.project.vcsProcessed.copy(
                url = "<REPLACE_URL_PROCESSED>",
                path = "<REPLACE_PATH>",
                revision = "<REPLACE_REVISION>"
            )
            val clonedProject = result.project.copy(
                vcsProcessed = vcsProcessed
            )
            val clonedResult = result.copy(
                project = clonedProject
            )
            expectedResultFile.writeText(clonedResult.toYaml())
        }
