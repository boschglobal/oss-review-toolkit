...
internal object NuGetInspector : CommandLineTool {
    ...
    fun run(workingDir: File, definitionFile: File, nugetConfig: String?): Result {
...
internal fun NuGetInspector.Result.toOrtProject(
    ...
    val nestedPackages: MutableList<NuGetInspector.PackageData> = mutableListOf()

    packages.forEach {
        it.dependencies.forEach { pkg -> nestedPackages.add(pkg) }
    }
...
internal fun Set<NuGetInspector.PackageData>.toOrtPackages(): Set<Package> =
    groupBy { "${it.name}:${it.version}" }.mapTo(mutableSetOf()) { (_, packages) ->
        ...
        fun NuGetInspector.PackageData.getHash(): Hash = Hash.create(
            sha512?.lowercase() ?: sha256?.lowercase() ?: sha1?.lowercase() ?: md5?.lowercase() ?: ""
        )
