...
import org.ossreviewtoolkit.model.config.LocationWithFalsePositives
...
class FossIdSnippetChoiceTest : WordSpec({
    ...
    "scanPackages()" should {
        "report a file with a snippet choice as pending if it has other non-chosen snippets" {
        ...
        "mark a file with all snippets chosen as identified" {
            ...
            val snippetChoices = createPackageSnippetChoices(
                vcsInfo.url,
                SnippetChoice(choiceLocation, "", "MIT".toSpdx(), PURL_1)
            )
            val fossId = createFossId(config)
        ...
        "create an issue if the chosen snippet is not in the snippet results" {
            ...
            summary.issues.forAtLeastOne {
                it.message shouldBe "Snippet choice's snippet $PURL_1 for ${choiceLocation.prettyPrint()} " +
        ...
        "add the license of marked as identified files that have been manually marked in the UI (legacy behavior)" {
            ...
            val summary = fossId.scan(createPackage(pkgId, vcsInfo), packageSnippetChoices = emptyList()).summary
        ...
        "put a marked as identified file back to pending if some of its snippet choice have been deleted" {
            val projectCode = projectCode(PROJECT)
            val scanCode = scanCode(PROJECT, null)
            val config = createConfig(deltaScans = false, fetchSnippetMatchedLines = true)
            val vcsInfo = createVcsInfo()
            ...
            summary.issues.filter { it.severity > Severity.HINT } should beEmpty()
            // Two snippets have been removed by the snippet choice. Only one snippet remain.
            summary.snippetFindings shouldHaveSize 1
            summary.snippetFindings.last().apply {
                sourceLocation.path shouldBe FILE_1
                snippets shouldHaveSize 1
...
private fun createPackageSnippetChoices(provenanceUrl: String, vararg snippetChoices: SnippetChoice) =
    listOf(PackageSnippetChoice(provenanceUrl, snippetChoices.toList()))

private fun createLocationWithFalsePositives(
    provenanceUrl: String,
    vararg locationWithFalsePositives: LocationWithFalsePositives
) = listOf(PackageSnippetChoice(provenanceUrl, emptyList(), locationWithFalsePositives.toList()))
